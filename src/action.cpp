...
void action_command(ParsedInputData* data) {
  ...
  for (unsigned i = 0; i < data->getDataCommandChain().size(); i++) {
    ...
    // now prepare Pipe-Objects
    {
      ...
      if (pipe_to_next.has_value()) {
        ...
        pipe_to_current.emplace(pipe_to_next.value());
        pipe_to_next.reset();
      }
    ...
    if (pid > 0) {
      ...
      // this is really important, otherwise the whole application (chain) could
      // stuck when the internal 64KB buffer (Linux) is full, e.g. when 'cat <
      // in_65kb.txt | grep -i hi' this is, because Linux thinks that a reader
      // (the parent) exists but it never reads from the buffer -> deadlock
    ...
    // child: this code is responsible for connecting it's stdin and stdout with
    // the proper fd's from the pipes;
    else {
      // Everything here is now done in a separately address space (because we
      // forked)
  ...
  if (!data->getDataCommandChain().isBackground()) {
    ...
    for (unsigned i = 0; i < data->getDataCommandChain().size(); i++) {
      bg_processes.push_back(new BgProcessState(pids[i]));
    }
  }
}
 ...
 * @param data parsed input data
 */
void action_alias([[maybe_unused]] ParsedInputData* data,
                  [[maybe_unused]] InputKind aliasKind) {
  // TODO
}
