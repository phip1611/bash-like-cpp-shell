...
std::string normalize_input(char const* const input) {
  ...
  // str_len == spaces_at_begin  is alias of == spaces_at_end in the case of
  // only spaces
...
bool str_starts_with(std::string const* const haystack,
                     std::string const* const needle) {
...
bool str_contains_single_word_from_to(const std::string* const input,
                                      size_t from,
                                      size_t to) {
  assert(from <= to);
  assert(to <= input->length());
  // assert(from <= input->length()); already proved because of the partial
  // order of from, to and input length :)
...
bool str_contains_only_given_chars(const std::string* const input,
                                   size_t from,
                                   size_t to,
...
std::vector<std::string> str_split_str(const std::string* s,
...
std::vector<std::string> str_split_char(const std::string* s,
                                        char const delim) {
  // code from https://stackoverflow.com/a/46931770/2891595
  std::vector<std::string> res;
...
{ std::smatch m; bool found; try { found = std::regex_match(str, m, regex); }
