...
ParsedInputData parse(std::string const* const normalized_input) {
  ...
  try {
    if (*normalized_input == "exit") {
      ...
      data.setDataCdDir(std::optional(parse_cd_data(normalized_input)));
      ...
      data.setDataCommandChain(parse_command_chain(normalized_input));
    } else {
      data.setType(InputKind::UNKNOWN);
    }
  } catch (std::exception& e) {
    std::cerr << "Error during parsing of command: Input type unknown!"
              << std::endl;
    data.setType(InputKind::UNKNOWN);
  }
...
bool verify_is_un_alias_input(const std::string* const input) {
  ...
  if (!str_contains_only_given_chars(
          input,
          UN_ALIAS_COMMAND_PREFIX.length() + 1,  // skip space
          input->length(), &GET_ALIAS_NAME_ALLOWED_CHARS)) {
...
bool verify_is_set_alias_input(const std::string* const input) {
  ...
  if (!str_contains_only_given_chars(
          input,
          SET_ALIAS_COMMAND_PREFIX.length() + 1,  // skip space
          eq_sign_i, &GET_ALIAS_NAME_ALLOWED_CHARS)) {
    // format of alias illegal
    fprintf(stderr, "Syntax is: alias <alias-name>=<alias-value>\n");
    return false;
  }
  ...
  // check value format
  if (!str_contains_only_given_chars(input, eq_sign_i + 1, input->length(),
...
CommandChain parse_command_chain(const std::string* const input) {
  ...
  for (size_t i = 0; i < basic_command_strs.size(); i++) {
    basic_commands.push_back(parse_command_chain_command(
        &basic_command_strs[i], i, basic_command_strs.size()));
  }
...
Command parse_command_chain_command(const std::string* const input,
                                    size_t i,
                                    size_t n) {
  Command cmd;
  ...
  if (check_executable_path_exists(&basic_command_string_cmd)) {
    ...
    std::cerr << "Can't find executable path for command '"
              << basic_command_string_cmd << "'" << std::endl;
    throw std::exception();
  }
...
void parse_command_chain_command_io_redirection(
    Command& cmd,
    const std::string* const basic_command_str) {
  // only supported if:
  // input file && is first
  // output file && is last
  if (cmd.is_in_middle())
...
bool check_executable_path_exists(std::string* command) {
  ...
  if ((*command)[0] == '/' || str_starts_with(command, &local_path_prefix) ||
      str_starts_with(command, &parent_path_prefix)) {
    return access(command->c_str(), F_OK | X_OK) != -1;
  }

  // otherwise search in path
  std::string path_var =
      getenv("PATH");  // calls the string constructor which creates a copy
