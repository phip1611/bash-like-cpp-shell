...
std::string& ParsedInputData::getDataAliasName() {
  assert(this->type == InputKind::GET_ALIAS ||
         this->type == InputKind::UN_ALIAS);
...
void ParsedInputData::setDataCdDir(
    const std::optional<std::string> dataCdDirNew) {
  ParsedInputData::dataCdDir = dataCdDirNew;
}

void ParsedInputData::setDataCommandChain(
    const std::optional<CommandChain> dataCommandChainNew) {
  ParsedInputData::dataCommandChain = dataCommandChainNew;
}

void ParsedInputData::setDataAliasName(
    const std::optional<std::string> dataAliasNameNew) {
  ParsedInputData::dataAliasName = dataAliasNameNew;
}

void ParsedInputData::setDataAliasValue(
    const std::optional<std::string> dataAliasValueNew) {
